...
def generate_gmm(input_folder, N):
    ...
    th = 1.0 / N
    means = np.float32([m for k,m in zip(range(0, len(weights)), means) if weights[k] > 0])
    covs = np.float32([m for k,m in zip(range(0, len(weights)), covs) if weights[k] > 0])
    weights = np.float32([m for k,m in zip(range(0, len(weights)), weights) if weights[k] > 0])
...
loadgmm=False
working_folder="E:/Test0"
number=5
gmm = load_gmm(working_folder) if loadgmm else generate_gmm(working_folder, number)
